...
public class AddCalExpenseFileCommand : IRequest<Unit>
{
    public AddCalExpenseFileRequest Request { get; set; } = null!;

    public class AddCalExpenseFileCommandHandler : IRequestHandler<AddCalExpenseFileCommand, Unit>
    {
        private readonly ApplicationDbContext _dbContext;

        public AddCalExpenseFileCommandHandler(ApplicationDbContext dbContext)
        {
            _dbContext = dbContext;
        }

        public async Task<Unit> Handle(AddCalExpenseFileCommand cmd, CancellationToken cancellationToken)
        {
            ...
            csv.Context.RegisterClassMap<CalCardExpenseRecord.CsvMap>();
            var records = csv.GetRecords<CalCardExpenseRecord>().ToList();

            var report = new CalCardExpenseReport()
            {
            ...
            _dbContext.CalCardExpenseReports.Add(report);
